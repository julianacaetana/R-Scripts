...
for (i in 1:length( cadastro$cadastros)) {
  ...
  data.frame(cnpj = str_pad(as.character(cadastro$cadastros[[i]]$id),8,"left",'0'),
             nome = cadastro$cadastros[[i]]$n,
             fcnpj =   if(is.null(cadastro$cadastros[[i]]$f$cnpj )) "N/A"  else str_pad(as.character(cadastro$cadastros[[i]]$f$cnpj),8,"left",'0') 
...
for (i in 1:nrow(cadastro.instituicoes)) {
 tryCatch({ifs_balancetes_cnpj(cadastro.instituicoes$cnpj[i],database)},error = function(e) {
