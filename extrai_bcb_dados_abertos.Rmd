...
cadastro <- fromJSON(readLines("https://www3.bcb.gov.br/informes/rest/cadastros"))

cadastro.instituicoes <- data.frame() 
for (i in 1:length( cadastro$cadastros)) {
  cadastro.instituicoes  <- bind_rows(cadastro.instituicoes,
  data.frame(cnpj = as.character(substr( paste("00000000", cadastro$cadastros[[i]]$id, sep = ""),9,16)),
             nome = cadastro$cadastros[[i]]$n))
}
